...
_date_alt = _whole_word(r'\d{2} (Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec) \d{4}')
...
def dates_iso8601(text):
    '''Find tokens that are in a date format.'''
    for match in _date_iso8601_pat.finditer(text):
        yield('date', match)

def dates_alt(text):
    for match in _date_alt.finditer(text):
        yield('date', match)       
